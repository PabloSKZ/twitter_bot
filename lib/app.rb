...
def upload_a_tweet(tweet)
    login_twitter.update(tweet)
end

def send_group(nb, journalists)
    group = journalists.sample(nb)
    group.each {|i| upload_a_tweet("Bonjour #{i}, votre dévotion à la cause journalistique vous honore. Si votre ramage égale votre plumage, être cité par votre plume est un hommage. #bonjour_monde")}
end

def search_by_hashtag(hashtag, nb)
    search_arr = []
    login_twitter.search(hashtag).take(nb).each do |tweet|
        search_arr << tweet.inspect
    end
    return search_arr.map {|x| x[/\d+/]}  
end

def favorite_by_hashtag(hashtag, nb)
    search_by_hashtag(hashtag, nb).each {|i| login_twitter.favorite(i) }
end

def follow_by_hashtag(hashtag, nb)
    j = 0
    user_arr = []
    login_twitter.search(hashtag).take(100).each do |tweet|
        if !user_arr.include?(tweet.user)
            user_arr << tweet.user
            puts tweet.user.inspect
            login_twitter.follow(tweet.user) if tweet.user.inspect != "#<Twitter::User id=1217040648915902466>"
            j += 1
            break if j > nb
        end
    end
